...
import { MeasureMetricsContext, newMetrics, type Branding, type Tx, type WorkspaceId } from '@hcengineering/core'
...
const metricsContext = initStatisticsContext('indexer', {
  factory: () =>
...
Analytics.setTag('application', 'indexer')
...
const config: (workspace: WorkspaceId, branding: Branding | null) => FulltextDBConfiguration = (
  workspace,
  branding
) => ({
  fulltextAdapter: {
    ...
    stages: (adapter, storage, storageAdapter, contentAdapter) =>
      createIndexStages(
        metricsContext.newChild('stages', {}),
        workspace,
        branding,
        adapter,
        storage,
...
if (serverSecret === undefined) {
  metricsContext.info('Please provide server secret')
  process.exit(1)
}
